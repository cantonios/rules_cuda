...
load("//cuda/private:toolchain_configs/utils.bzl", "hipcc_version_ge")
...
def _impl(ctx):
    ...
    hipcc_compile_env_feature = feature(
        name = "hipcc_compile_env",
        env_sets = [
            env_set(
    ...
    hipcc_device_link_env_feature = feature(
        name = "hipcc_device_link_env",
        env_sets = [
            env_set(
    ...
    hipcc_allow_unsupported_compiler_feature = feature(
        name = "hipcc_allow_unsupported_compiler",
        flag_sets = [
            flag_set(
                actions = [
                    ACTION_NAMES.cuda_compile,
    ...
    hipcc_relaxed_constexpr_feature = feature(
        name = "hipcc_relaxed_constexpr",
        enabled = True,
        flag_sets = [
            flag_set(
                actions = [
    ...
    hipcc_extended_lambda_feature = feature(
        name = "hipcc_extended_lambda",
        enabled = True,
        flag_sets = [
            flag_set(
                actions = [
                ...
                flag_groups = [flag_group(flags = ["--extended-lambda" if hipcc_version_ge(ctx, 10, 2) else "--expt-extended-lambda"])],
            ),
        ],
    )
    ...
    features = [
        hipcc_compile_env_feature,
        hipcc_device_link_env_feature,
        arch_native_feature,
        pic_feature,
        ...
        hipcc_allow_unsupported_compiler_feature,
        hipcc_relaxed_constexpr_feature,
        hipcc_extended_lambda_feature,
...
cuda_toolchain_config = rule(
    doc = """This rule provides the predefined cuda toolchain configuration for hipcc with non MSVC as host compiler.""",
    implementation = _impl,
    attrs = {
        "cuda_toolkit": attr.label(mandatory = True, providers = [CudaToolkitInfo], doc = "A target that provides a `CudaToolkitInfo`."),
        "toolchain_identifier": attr.string(values = ["hipcc"], mandatory = True),
        "hipcc_version_major": attr.int(doc = "The CUDA Toolkit major version, e.g, 11 for 11.6"),
        "hipcc_version_minor": attr.int(doc = "The CUDA Toolkit minor version, e.g, 6 for 11.6"),
